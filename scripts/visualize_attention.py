...
def visualize_cross_attention(model, visualizer, image, text_tokens, output_path, image_path=None):
    ...
    if image_path and os.path.exists(image_path):
        ...
        overlay_attn = (overlay_attn - overlay_attn.min()) / (overlay_attn.max() - overlay_attn.min() + 1e-8)
        overlay_attn = np.array(Image.fromarray((overlay_attn * 255).astype(np.uint8)).resize((224, 224)))
...
def visualize_attention_divergence(visualizer, attn_weights, output_path):
    ...
    for head_idx in range(num_heads):
        ...
        score = visualizer.fast_epps_pulley(attn_head.flatten(), attn_head.flatten())
...
def main():
    parser = argparse.ArgumentParser(description='Visualize attention patterns for MicroVLM-M')
    ...
    batch_visualize(model, visualizer, dataset, args.num_samples, args.output_dir)
