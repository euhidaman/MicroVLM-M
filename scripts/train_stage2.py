...
class Stage2Trainer:
    def __init__(self, config_path, checkpoint_path=None):
        ...
        if checkpoint_path:
            ...
            print(f"Loading Stage 1 checkpoint from {self.config['stage1_checkpoint']}")
    ...
    def _setup_freezing(self):
        ...
        unfreeze_last_n = self.config['training'].get('unfreeze_last_n_lm_layers', 4)
        ...
        print(f"Frozen LM layers: {freeze_lm}/{self.model.language_model.config['num_layers']}")
        ...
        print(f"Unfrozen last {unfreeze_last_n} LM layers (layers {unfreeze_start}-{total_layers-1})")
    ...
    def train_step(self, batch):
        ...
        total_loss, lm_loss, memory_loss, scope_loss, scope_decision = self.compute_loss(batch)
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            epoch_pbar = tqdm(self.train_loader, desc=f"Epoch {epoch+1}/{num_epochs}")
...
def main():
    parser = argparse.ArgumentParser(description='Stage 2 Training for MicroVLM-M')
