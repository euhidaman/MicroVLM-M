...
def verify_memory_loading(memory_path, config_path):
    ...
    print(
        f"W_M parameters: {sum(p.numel() for p in memory.W_M.parameters()):,}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Export episodic memory module')
    ...
    memory_state = export_memory_module(
        args.checkpoint, args.output, args.config)
