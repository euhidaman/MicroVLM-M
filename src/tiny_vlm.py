...
class TinyVLM(nn.Module):
    ...
    def __init__(self, config_path=None, device='cuda'):
        ...
        self.episodic_memory = EpisodicMemory(
            config_dict={'memory': self.config['memory'], 'bitnet': self.config['bitnet']},
            device=device
        )
        ...
        self.attention_visualizer = AttentionVisualizer(config_dict=self.config['attention_viz'])
    ...
    def build_fused_sequence(self, prefix_tokens, text_token_ids):
        ...
        fused_embeds = torch.cat([bos_embeds, prefix_tokens, text_embeds, eos_embeds], dim=1)
    ...
    def forward(self, images, text_token_ids, use_memory=True, return_attention=False):
        ...
        if use_memory and scope_decision.sum() > 0:
            ...
            for layer_idx in range(self.config['bitnet']['num_layers']):
                mem_k, mem_v = self.episodic_memory.inject_to_kv_cache(Z_r_kv, layer_idx)
