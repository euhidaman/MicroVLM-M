...
class AttentionVisualizer(nn.Module):
    ...
    def extract_cross_attention(self, attention_weights, image_token_range, text_token_range):
        ...
        cross_attn = attention_weights[:, :,
                                       img_start:img_end, text_start:text_end]
    ...
    def compute_attention_divergence(self, cross_attn):
        ...
        # (batch * num_heads, num_img * num_text)
        attn_flat = cross_attn.reshape(-1, num_img * num_text)

        # Apply slicing test
        divergence = self.slicing_test(
            attn_flat.unsqueeze(1))  # Add sample dimension
    ...
    def generate_heatmap(self, cross_attn, save_path=None, title="Cross-Attention Heatmap"):
        ...
        # Plot heatmap
        im = ax.imshow(cross_attn, cmap='viridis',
    ...
    def generate_memory_heatmap(self, memory_addressing, save_path=None, title="Memory Addressing Heatmap"):
        ...
        # Plot heatmap
        im = ax.imshow(memory_addressing, cmap='plasma',
                       aspect='auto', interpolation='nearest')
...
if __name__ == "__main__":
    ...
    fig = visualizer.generate_heatmap(
        sample_attn, title="Test Cross-Attention")
    plt.close(fig)
    ...
    fig = visualizer.generate_memory_heatmap(
        memory_weights, title="Test Memory Addressing")
    plt.close(fig)
