...
class MultimodalAdapter(nn.Module):
    ...
    def __init__(self, config_path=None, config_dict=None):
        ...
        if self.num_patches % self.k_prefix != 0:
            ...
            self.pool_query = nn.Parameter(
                torch.randn(1, self.k_prefix, self.output_dim))
    ...
    def forward(self, patch_embeddings):
        ...
        # (batch, num_patches, output_dim)
        x = self.projection(patch_embeddings)
        ...
        if self.use_attention_pool:
            ...
            # (batch, k_prefix, output_dim)
            pooled, _ = self.pool_attn(query, x, x)
        else:
            # Simple average pooling over groups
            x_reshaped = x.view(batch_size, self.k_prefix,
                                self.pool_size, self.output_dim)
