...
class EpisodicMemory(nn.Module):
    ...
    def __init__(self, config_path=None, config_dict=None, device='cuda'):
        ...
        self.pseudoinverse_approx_step = config.get('pseudoinverse_approx_step', 3)
        ...
        if self.w_logvar_setting == 0:
            ...
            self.w_logvar = nn.Parameter(torch.zeros(self.k_mem), requires_grad=True)
    ...
    def read(self, z_query, memory_state, deterministic=None):
        ...
        z_retrieved = torch.bmm(w_exp, memory_mean).squeeze(1)  # (batch, C_mem)
    ...
    def inject_to_kv_cache(self, Z_r_kv, layer_idx):
        ...
        mem_k = kv_reshaped[:, layer_idx, 0, :, :].unsqueeze(2)  # (batch, num_heads, 1, head_dim)
        mem_v = kv_reshaped[:, layer_idx, 1, :, :].unsqueeze(2)  # (batch, num_heads, 1, head_dim)
    ...
    def _compute_kl_divergence(self, prior_memory, posterior_memory):
        ...
        t2 = torch.sum((post_mean - prior_mean) ** 2 / prior_var.unsqueeze(-1), dim=[-2, -1])
        t3 = -self.c_mem * self.k_mem
        t4 = self.c_mem * torch.sum(torch.log(prior_var) - torch.log(post_var), dim=-1)
...
if __name__ == "__main__":
    ...
    print(f"Read test - Retrieved shape: {z_ret.shape}, KV shape: {z_kv.shape}")
