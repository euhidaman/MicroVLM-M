...
class CC12MDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.tokenizer is not None:
            tokens = self.tokenizer(caption, max_length=self.max_text_length, truncation=True)
    ...
    def _char_tokenize(self, text):
        ...
        token_ids = [ord(c) - ord('a') + 1 if c.isalpha() else 27 for c in text]
        # Pad to max length
    ...
    def collate_fn(batch):
        ...
        token_ids = torch.stack([
            torch.cat([item['token_ids'], torch.zeros(max_len - item['token_ids'].size(0), dtype=torch.long)])
            for item in batch
        ])
...
if __name__ == "__main__":
    ...
    if os.path.exists(metadata_path):
        ...
        loader = DataLoader(dataset, batch_size=4, collate_fn=CC12MDataset.collate_fn)
