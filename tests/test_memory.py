...
def test_memory_initialization():
    ...
    assert memory.memory.shape == (num_slots, slot_dim), f"Wrong memory shape: {memory.memory.shape}"
...
def test_memory_write():
    ...
    assert not torch.allclose(memory_before, memory_after), "Memory not updated after write"
    
    print(f"  Input shape: {z_t.shape}")
    print(f"  Memory updated: {not torch.allclose(memory_before, memory_after)}")
...
def test_memory_read():
    ...
    assert addressing_weights.shape == (2, 128), f"Wrong addressing shape: {addressing_weights.shape}"
    
    weights_sum = addressing_weights.sum(dim=1)
    assert torch.allclose(weights_sum, torch.ones(2), atol=1e-5), f"Addressing weights don't sum to 1: {weights_sum}"
    
    print(f"  Query shape: {query.shape}")
    print(f"  Z_r shape: {Z_r.shape}")
    print(f"  Addressing weights shape: {addressing_weights.shape}")
    print(f"  Weights sum to 1: {torch.allclose(weights_sum, torch.ones(2), atol=1e-5)}")
...
def test_memory_kv_injection():
    ...
    assert kv_cache.shape == (2, 153600), f"Wrong KV cache shape: {kv_cache.shape}"
...
def test_memory_save_load():
    ...
    try:
        ...
        assert torch.allclose(memory1.memory, memory2.memory), "Memory not loaded correctly"
        
        print(f"  Saved to: {temp_path}")
        print(f"  Memory matches after load: {torch.allclose(memory1.memory, memory2.memory)}")
