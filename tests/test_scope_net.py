...
def test_scope_net_output():
    ...
    assert decision.shape == (batch_size,), f"Wrong decision shape: {decision.shape}"
    assert probability.shape == (batch_size,), f"Wrong probability shape: {probability.shape}"
    
    assert (probability >= 0).all() and (probability <= 1).all(), "Probabilities out of [0, 1] range"
    ...
    print(f"  Probability range: [{probability.min():.4f}, {probability.max():.4f}]")
    print(f"  Decisions are binary: {((decision == 0) | (decision == 1)).all()}")
